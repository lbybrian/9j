  ...
  <div class="addRule">
    ...
    <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm" size="small" autocomplete =‘true’>
      ...
      <el-form-item v-if="active===1" label="规则">
        <configuration @config="config" :ruleForm="ruleForm" ref="formData"></configuration>
      </el-form-item>
...
//      this.ruleForm=sessionStorage.getItem('ruleForm')?sessionStorage.getItem('ruleForm'):'';
        console.log(this.ruleForm.formData.timeRangeSign,'上一步')
        if (this.active-- < 0) this.active = 0
      		this.ruleForm.formData=this.$store.state.formData?this.$store.state.formData:this.ruleForm.formData
      },
      // 下一步
      nextStep() {
