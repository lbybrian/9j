...
//      immediate:true,
    ...
    created() {
//      	console.log('$2222222222222222',this.ruleForm)
...
//未实现二	
if(this.ruleForm.cycleStatTastDetailDTO&&this.ruleForm.cycleStatTastDetailDTO.startTime&&this.ruleForm.cycleStatTastDetailDTO.endTime){
	this.formData.showTime.push(this.ruleForm.cycleStatTastDetailDTO.startTime)
	this.formData.showTime.push(this.ruleForm.cycleStatTastDetailDTO.endTime)
}
    	...
    	for(let i=0;i<this.formData.bucketBO.aggregationBOList.length;i++){
    		this.formData.bucketBO.aggregationBOList[i].arr=[]
    		this.formData.bucketBO.aggregationBOList[i].arr[0]=this.formData.bucketBO.aggregationBOList[i].fieldName
    		this.formData.bucketBO.aggregationBOList[i].arr[1]=this.formData.bucketBO.aggregationBOList[i].aggSign
//  		this.formData.bucketBO.aggregationBOList[i].arr[2]=this.formData.bucketBO.aggregationBOList[i].aggName

//				this.formData.bucketBO.aggregationBOList[i].arr[0]=this.dict['012']['dataValue'][i].value
//  		this.formData.bucketBO.aggregationBOList[i].arr[1]=this.dict['012']['dataValue'][i].children[0].value

//  		this.formData.bucketBO.aggregationBOList[i].arr[0]=this.dict['012']['dataValue'][i].value
//  		this.formData.bucketBO.aggregationBOList[i].arr[1]=this.dict['012']['dataValue'][i].children[0].value
    	}
//  	console.log(11111111111,this.formData.bucketBO.aggregationBOList,222222222,this.dict['012']['dataValue'])
    	...
    	for(let i=0;i<this.formData.queryANDList.length;i++){
//  		this.formData.queryANDList[i].and[0]=[{fieldName: '',aggName: '',aggSign: '',}]
    		this.formData.queryANDList[i].and=[]
	    		this.formData.queryANDList[i].and[0]=this.formData.queryANDList[i].fieldName
	    		this.formData.queryANDList[i].and[1]=this.formData.queryANDList[i].aggSign
	    		this.formData.queryANDList[i].and[2]=this.formData.queryANDList[i].aggName
    	}
    	
//  	if(this.formData.queryORList.length>=0){
//  		for(let i=0;i<=this.formData.queryORList.length;i++){
//  			this.formData.queryORList[i].or=[]
//  		}
//  	}
    	
    	console.log('子表单1',this.formData,this.ruleForm.queryRuleJson)
    	
        //桶外聚合
//  	for(let i=0;i<this.formData.aggOutList.length;i++){
    	for(let i in this.formData.aggOutList){
    		this.formData.aggOutList[i].arr=[]
    		
        // cascader 回显部门处理，只处理两级，如果多于两级需要改写成递归函数
        this.dict['012']['dataValue'].map(item => {
          if(item.value == this.formData.aggOutList[i].fieldName){
            this.formData.aggOutList[i].arr.push(item.value)
          }else if(item.children && item.children.length > 0){
            for( let i in item.children){
              if(item.children[i].value == this.formData.aggOutList[i].aggSign){
                this.formData.aggOutList[i].arr.push(item.children[i].value)
              }
            }
          }
        })
...
//  	this.formData.showTime.push(this.ruleForm.cycleStatTastDetailDTO.startTime)
//  	this.formData.showTime.push(this.ruleForm.cycleStatTastDetailDTO.endTime)
    	console.log('子表单',this.formData,this.ruleForm.queryRuleJson)
    },
    mounted() {},
    beforeDestroy(){},
    methods: {
