...
<script>
    ...
    watch: {
      ...
      ruleForm: {
        handler(newValue, oldValue) {
          ...
          console.log('alarmRule页面',newValue)
        },
        deep: true,
        immediate:true
      },
    ...
    created() {
//  	console.log(1111111111,this.alls,this.ruleForm,this.obj,this.obj2)
    	console.log(1111111111,this.ruleForm)
...
//  		return obj
//  	}
//  	this.ruleForm=deepClone(this.alls)
    	
    	
    	for(let item of this.ruleForm.rows){
    		
        this.alls.aardList.map(dat => {
        	//一一对应，因为绑定的数据格式与付传志格式一致
        	
          if(dat.value == item.fieldName){
            item.arr.push(dat.value)
          }else if(dat.children && dat.children.length > 0){
            for( let i in dat.children){
              if(dat.children[i].value == item.aggSign){
                item.arr.push(dat.children[i].value)
              }
            }
          }
        })
...
//  	console.log(2222222,this.alls,this.ruleForm,this.obj,this.obj2)
